...
public:
	...
	void grabarEnArchivoVenta(Venta obj)
	{
		try
		{
			...
			{
				archivoVenta<<obj.getCodVenta()<<";"<<obj.getCodCliVen()<<";"<<obj.getFechaVen()<<";"<<obj.getSerieVen()<<";"<<obj.getTotalVen()<<";"<<endl;
				archivoVenta.close();					
			}
		}
		catch(exception e)
	...
	void grabarModificarEliminarArchivoVenta()
	{
		try
		{
			...
			if(archivoVenta.is_open())
			{
				...
				{
					archivoVenta<<x.getCodVenta()<<";"<<x.getCodCliVen()<<";"<<x.getFechaVen()<<";"<<x.getSerieVen()<<";"<<x.getTotalVen()<<";"<<endl;
				}
				archivoVenta.close();					
			}
		}
			...
			if(archivoVenta.is_open())
			{
				while(!archivoVenta.eof() && getline(archivoVenta,linea))
				{
					...
					Venta objVenta(stoi(temporal[0]),stoi(temporal[1]),
									   temporal[2],stoi(temporal[3]),stof(temporal[4]));
